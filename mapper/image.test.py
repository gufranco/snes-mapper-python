...
class InterleaveTest(unittest.TestCase):
    ...
    def test_the_lower_half_of_a_bank_moves_behind_every_upper_half(self):
        logical = a_logical(2)
    ...
    def test_an_image_that_is_not_whole_banks_is_refused(self):
        with self.assertRaises(image.NotWholeBanks):
            image.interleave(bytes(image.BANK + 3))


class AddressTest(unittest.TestCase):
    def test_the_upper_half_of_a_bank_sits_in_the_first_run(self):
        self.assertEqual(image.snes_to_file(0, image.HALF, 4), 0)

    def test_the_lower_half_of_a_bank_sits_behind_every_upper_half(self):
        self.assertEqual(image.snes_to_file(0, 0, 4), 4 * image.HALF)

    def test_each_bank_advances_by_half_a_bank(self):
        first = image.snes_to_file(0, image.HALF, 4)
        second = image.snes_to_file(1, image.HALF, 4)

        self.assertEqual(second - first, image.HALF)

    def test_an_address_and_its_offset_agree_in_both_directions(self):
        banks = 8
        for bank in range(banks):
    ...
    def test_every_offset_in_an_image_maps_back_to_an_address(self):
        banks = 4
...
class WindowTest(unittest.TestCase):
    def test_the_window_starts_where_the_mapper_puts_it(self):
        self.assertEqual(image.WINDOW_FIRST_BANK, 0xC0)

    def test_a_windowed_bank_reaches_the_image(self):
        found = image.window_to_file(image.WINDOW_FIRST_BANK, image.HALF, 8)

        self.assertGreaterEqual(found, 0)

    def test_the_two_halves_of_a_windowed_bank_come_from_different_runs(self):
    ...
    def test_a_windowed_bank_advances_by_half_a_bank(self):
    ...
    def test_an_address_below_the_window_takes_the_ordinary_route(self):
        self.assertEqual(
            image.address_to_file(0, image.HALF, 8), image.snes_to_file(0, image.HALF, 8)
        )

    def test_an_address_inside_the_window_takes_the_windowed_route(self):
    ...
    def test_every_windowed_bank_lands_inside_an_image_that_holds_it(self):
        banks = 0x40
