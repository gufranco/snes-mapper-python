...
class CopierStubTest(unittest.TestCase):
    ...
    def test_a_file_shorter_than_the_stub_cannot_be_one(self) -> None:
        self.assertFalse(dump.has_copier_stub(bytes(dump.COPIER_BYTES)))

    def test_a_file_of_no_recognised_length_is_left_alone(self) -> None:
...
class GameDoctorTest(unittest.TestCase):
    ...
    def test_a_folder_with_no_parts_gives_nothing_rather_than_failing(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            (Path(folder) / "notes.txt").write_bytes(b"nothing here")

            self.assertEqual(dump.join_game_doctor(Path(folder)), b"")
