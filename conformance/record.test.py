...
class FieldsTest(unittest.TestCase):
    def test_a_case_carries_the_numbers_and_not_the_title(self):
    ...
    def test_a_case_carries_the_offset_the_header_sat_at(self):
        found = header.read(a_cartridge())

        self.assertEqual(record.fields_of(found)["at"], header.LOROM_HEADER)

    def test_a_copier_stub_does_not_change_the_offset_a_case_carries(self):
...
class StubTest(unittest.TestCase):
    def test_a_stubbed_dump_records_the_offset_the_cartridge_has(self):
    ...
    def test_a_stubbed_dump_and_a_bare_one_are_the_same_case(self):
        pair = [a_cartridge(), b"\x00" * header.COPIER_BYTES + a_cartridge()]

        self.assertEqual(len(record.gather(pair, PROBES)[0]), 1)

    def test_what_a_stubbed_dump_records_replays_clean(self):
...
class ReplayTest(unittest.TestCase):
    def test_what_was_recorded_replays_without_disagreeing(self):
...
class MainTest(unittest.TestCase):
    ...
    def test_a_limit_stops_after_that_many_cartridges(self):
        record.main([str(self.where), str(self.out), "1"])

        self.assertEqual(json.loads(self.out.read_text())["measured_from"], 1)

    def test_the_probes_of_an_existing_corpus_are_kept(self):
    ...
    def test_a_directory_with_no_cartridges_is_refused(self):
