...
**What Nintendo printed, and what follows from it.** The development manual gives
the two bus speeds in its Map Mode table and no table of access cycles per
region. Two of the three access figures this project uses follow from those
speeds by arithmetic that is shown rather than asserted, and the third does not
follow from anything printed and is marked unverified with the measurement that
would settle it.

**Where the evidence is a cartridge rather than a document.** Header field
combinations are read out of a library of retail cartridges, every distinct
combination checked against the layout and the addresses it produces, and every
cartridge then read again and held to the case that recorded it. No cartridge is
carried here and none ever will be: `conformance/record.py` rebuilds the corpus
from a library you already own, and the corpus that ships is what it produced.

**This member is a board rather than a part.** There is no clock to drive and no
processor to hand back, so the sections of the standard about those do not apply
and the rest does. What it answers is which address a read lands on and who is
allowed to answer it, which was the layer every defect in a year of this work
turned out to live in.

**It names the machine, and that is correct here.** A cartridge memory map only
ever existed as part of one machine. The rule about not naming a machine is
about parts that outlived their first customer, and this is not one of them.
