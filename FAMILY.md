...
Everything in the table above is in place. What a header field means comes from
Nintendo's own manual; what cartridges actually carry comes from a library of
several thousand, and the two are kept apart rather than merged.

**Two of the three access counts are derived, not inherited.** Book I prints two
bus speeds and no cycle table. Six master cycles is 3.5795 MHz, which is the
3.58 MHz the manual prints; eight is 2.6847 MHz, which is its 2.68 MHz. Both
figures therefore come from the document by arithmetic that is shown rather than
from an implementation that happens to agree.

**The third is marked unverified and stays that way.** Twelve master cycles for
the controller-port region is what every implementation uses and is almost
certainly right, and Book I lists two bus speeds rather than three. It carries
`verified: false` in [`conformance/hardware.json`](conformance/hardware.json) and
an entry in [`conformance/divergences.json`](conformance/divergences.json), so a
reader cannot inherit it as though it stood beside the other two.

**One step of the derivation is not from the manual either.** Turning a printed
megahertz figure into master cycles needs the master clock, and 21477273 Hz comes
from the NTSC colour carrier rather than from Nintendo. Named, for the same
reason.

**Nothing here is a CPU timing claim.** This package reports what one access to a
resolved address costs. What an instruction costs belongs to the processor, and
refresh, DMA stalls and everything else the console spends cycles on are outside
